//! Usage:
//!   pitch_test --list
//!   pitch_test --semitones 3 [--capture "CABLE Input"] [--out "<part of device name>"] [--secs 60]
...
const MAX_DEPTH_MS: usize = 500; // clock drift guard
const TRIM_TO_MS: usize = 200;
...
fn capture_thread(id: String, semitones: f64, sh: Arc<Shared>) -> Result<()> {
    let mut shifter = SoundTouch::new();
    shifter
        .set_channels(CHANNELS as u32)
        .set_sample_rate(RATE as u32)
        .set_pitch(2f64.powf(semitones / 12.0));
    let mut shifted = vec![0f32; 4096 * CHANNELS];
    ...
    unsafe {
        ...
        while !sh.stop.load(Ordering::Relaxed) {
            ...
            while cap.GetNextPacketSize()? > 0 {
                ...
                // Pitch shift
                shifter.put_samples(&samples, frames as usize);
                let mut out: Vec<f32> = Vec::new();
                let max_frames = shifted.len() / CHANNELS;
                loop {
                    let got = shifter.receive_samples(&mut shifted, max_frames);
                    if got == 0 {
                        break;
                    }
                    out.extend_from_slice(&shifted[..got * CHANNELS]);
                }
...
fn main() -> Result<()> {
    ...
    let semitones: f64 = arg(&args, "--semitones").and_then(|s| s.parse().ok()).unwrap_or(0.0);
    ...
    let t1 = std::thread::spawn(move || capture_thread(c, semitones, s1));
