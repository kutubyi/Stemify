//! Usage:
//!   stem_test --list
//!   stem_test [--mix backing] [--hop 0.4] [--lookahead 0.12] [--xfade 0.05] [--cushion 0.25]
...
fn worker_thread(model: PathBuf, look: usize, xf: usize, sh: Arc<Shared>) -> Res<()> {
...
fn main() -> Res<()> {
    ...
    let s = sh.clone();
    let worker = std::thread::spawn(move || worker_thread(model, look, xf, s));
